...
class FailureAnalyzer:
    ...
    def generate_improvement_suggestions(
        ...
        if categories.get("hallucination"):
            suggestions.append(
                "Implement a hallucination checker to filter claims not supported "
                "by the retrieved context and tighten grounding instructions."
            )
        if categories.get("irrelevant"):
            suggestions.append(
                "Add few-shot examples and clarify intent handling so answers "
        ...
        if categories.get("incomplete"):
            suggestions.append(
                ...
                "context fragmentation and capture more required conditions."
            )
        if categories.get("off_topic"):
            suggestions.append(
                "Review system prompt and intent routing to keep answers scoped "
                "to the topic being asked about."
            )

        fallback_suggestions = [
            "Cluster failures and prioritize fixes that resolve multiple cases at once.",
            "Expand the golden dataset with newly discovered failure cases for regression coverage.",
            "Calibrate the LLM judge against human labels to confirm scoring is trustworthy.",
        ]
